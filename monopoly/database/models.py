...
class Player(db.Model):
    ...
    id = db.Column(db.String(length=32), primary_key=True, nullable=False, doc='Unique user id')
    ...
    amount_of_railroads = db.Column(db.Integer, nullable=False, default=0,
                                    doc='The number of railways that the user has purchased')
...
class Card(db.Model):
    ...
    title = db.Column(db.String(length=64), nullable=False, doc='The name of the card in the game')
    color = db.Column(db.String(length=16), nullable=True, doc='Group of color cards in the game')
...
class Game(db.Model):
    ...
    id = db.Column(db.String(length=32), nullable=False, doc='Unique game id corresponding to the room id')
    player_id = db.Column(db.String(length=32), db.ForeignKey('players.id', ondelete='CASCADE'),
                          primary_key=True, nullable=False, doc='Unique user id')
    card_id = db.Column(db.Integer, db.ForeignKey('cards.id', ondelete='CASCADE'), primary_key=True,
                        nullable=False, doc='Unique card id in the game')
